...
class TextRegions(object):
    ...
    def filter(seg,area,label):
        ...
        ##good是area大于100的区域的数字化id(label)
        ##area是area大于100的区域(像素个数)
        ...
        aidx = np.argsort(-area)  ##从大到小排序
        good = good[filt][aidx]
        R = [R[i] for i in aidx]
        filter_info = {'label':good, 'rot':R, 'area': area[aidx]}
        ##返回的是符合条件的area，并且是从大到小排序的
        return filter_info
...
class RendererV3(object):
    ...
    def render_text(self,rgb,depth,seg,area,label,ninstance=1,viz=False):
        ...
        try:
            # depth -> xyz
            #将(H,W)的depth数组根据一定的公式算出X，Y都是H，W维度，然后和depth合并成(H，W，3)维度
